...
public class SQLiteHandler extends SQLiteOpenHelper{
    ...
    private static final String KEY_ID_LANGUAGE = "id";
    private static final String KEY_LANGUAGE = "choose_language";
    ...
    public void onCreate(SQLiteDatabase db) {
        ...
        String CREATE_LANGUAGE_TABLE = "CREATE TABLE " + TABLE_LANGUAGE + "("
                + KEY_ID_LANGUAGE + " INTEGER PRIMARY KEY," + KEY_LANGUAGE + " TEXT" + ")";
        db.execSQL(CREATE_LANGUAGE_TABLE);

        Log.d(TAG, "Database tables language created");
    }
    ...
    public void addLanguage(String chooseLanguage){
        SQLiteDatabase db = this.getWritableDatabase();

        ContentValues values = new ContentValues();
        values.put(KEY_LANGUAGE, chooseLanguage);

        // Inserting Row
        long id = db.insert(TABLE_LANGUAGE, null, values);
        db.close(); // Closing database connection

        Log.d(TAG, "New language inserted into sqlite: " + id);
    }
    ...
    public HashMap<String, String> getLanguageDetails() {
        ...
        cursor.moveToFirst();
        if (cursor.getCount() > 0) {
            language.put("languageName", cursor.getString(1));
        }
        cursor.close();
        db.close();
        // return user
        Log.d(TAG, "Fetching user from Sqlite: " + language.toString());
    ...
    public void deleteLanguage() {
        ...
        Log.d(TAG, "Deleted all language info from sqlite");
    }
